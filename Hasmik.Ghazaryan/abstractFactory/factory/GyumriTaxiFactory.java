...
public class GyumriTaxiFactory {

    public Taxi orderTaxi(ServiceType serviceType) {
        switch (serviceType) {
            case ECONOMY:
                Taxi economyTaxi  = new EconomyTaxi();
                economyTaxi.setIncludedDistance(Location.GYUMRI.includedDistance);
                return economyTaxi;
            case STANDARD:
                Taxi standardTaxi  = new StandardTaxi();
                standardTaxi.setIncludedDistance(Location.GYUMRI.includedDistance);
                return standardTaxi;
            case LUX:
                Taxi luxTaxi  = new LuxTaxi();
                luxTaxi.setIncludedDistance(Location.GYUMRI.includedDistance);
                return luxTaxi;
            default:
                throw new IllegalArgumentException(
                        String.format("Service type %s is not supported in Gyumri", serviceType.name));
